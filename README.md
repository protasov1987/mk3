...
### Render.com
- Используйте файл `render.yaml` из репозитория: он разворачивает веб-сервис с командой `npm start`, задаёт переменные `HOST=0.0.0.0` и `DATA_DIR=/data`, а также подключает Persistent Disk (1 ГБ) на путь `/data`.
