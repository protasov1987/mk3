...
Локальное веб-приложение для управления технологическими картами, маршрутами операций и трекингом выполнения. Данные сохраняются в файл `data/database.json` на локальном диске сервера, поэтому ими могут пользоваться несколько пользователей из разных браузеров и устройств, подключённых к одному экземпляру приложения. На платформах с эфемерным диском (например, Render без подключённого Persistent Disk) состояние сбрасывается при пересоздании инстанса.
...
- `db.js` — простой файловый "движок" базы данных (нормализация данных, клон перед обновлением, запись в `data/database.json`).
...
Данные создаются автоматически при первом запуске и хранятся в `data/database.json` рядом с приложением.
...
### Render.com
- Используйте файл `render.yaml` из репозитория: он разворачивает веб-сервис с командой `npm start` и задаёт переменную `HOST=0.0.0.0`.
- Данные пишутся в `data/database.json` на локальный диск инстанса Render. Без подключённого Persistent Disk этот каталог эфемерен и очищается при пересоздании инстанса; для долгосрочного хранения подключите внешний диск или используйте собственный сторедж.
