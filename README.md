...
### Render.com
- Используйте файл `render.yaml` из репозитория: он разворачивает веб-сервис с командой `npm start`, задаёт переменные `HOST=0.0.0.0` и `DATA_DIR=/data`, а также подключает Persistent Disk (400 МБ) на путь `/data`.
- При необходимости измените размер диска в `render.yaml` или через интерфейс Render. Все данные (вложенные файлы и база `database.json`) будут храниться на подключённом диске и переживут перезапуски.
- Если диск подключён в другое место, обновите переменную `DATA_DIR`, чтобы сервер писал базу именно туда.
